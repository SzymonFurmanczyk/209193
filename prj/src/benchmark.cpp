...
#define LENGTH 100//"ile razy pomnozymy przez 10 - patrz linijka 19 i 35"
#define REPEATS 4//ilosc powtorzen dla obliczenia sredniej - patrz linijka 20"
...
void Benchmark::testAlgorithm(Benchmark *_algorithm, int _n) const{
  ...
  if (ret_data.is_open()){
	...
	_algorithm -> runAlgorithmSzybkieSortowanie(); //wykonujemy algorytm
	
	std::chrono::high_resolution_clock::time_point end_time = std::chrono::high_resolution_clock::now();
	std::chrono::high_resolution_clock::duration time_period = end_time - start_time;

	std::cout << "Czas: " << std::chrono::duration_cast<std::chrono::microseconds>(time_period).count() << "mikrosek" <<" "<<j<< std::endl;
	average += std::chrono::duration_cast<std::chrono::microseconds>(time_period).count();//dodajemy czas dzialania algorytmu
      ...
      j+=100;//ilosc wczytywanych elementow
